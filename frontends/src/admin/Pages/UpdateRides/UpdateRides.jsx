...
const UpdateRides = () => {
  ...
  const fetchRideDetails = async () => {
    try {
      const response = await axiosInstanceAdmin.get(`/api/admin/rides/${id}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axiosInstanceAdmin.put(`/api/admin/rides/${id}`, formData, {
        withCredentials: true,
      });
      if (response.data.success) {
        toast.success(response.data.message);
        navigate("/admin/list"); // Redirect to ride list page (or wherever appropriate)
      } else {
