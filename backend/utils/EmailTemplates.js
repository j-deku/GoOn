...
const AdminInviteEmail = ({ name, inviteLink, expiresAt }) => `
<!DOCTYPE html>
<html>
<head>
  ...
  <style>
    ...
    .email-container { max-width: 600px; margin: 40px auto; background: #fff; border: 1px solid #e0e0e0; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); }
    .header { background-color: #1e88e5; color: #fff; padding: 20px; text-align: center; }
    .header img { max-width: 100px; margin-bottom: 10px; }
    .header h2 { margin: 0; font-size: 24px; font-weight: 600; }
    .body { padding: 20px 30px; color: #333; }
    .body h1 { color: #1e88e5; font-size: 22px; margin-top: 0; }
    .body p { margin: 10px 0; font-size: 16px; }
    .button { display: inline-block; background-color: #1e88e5; color: #fff; text-decoration: none; padding: 12px 20px; border-radius: 5px; font-weight: bold; text-align: center; font-size: 16px; margin: 20px auto 0; }
    .button:hover { background-color: #1565c0; }
    .footer { text-align: center; color: #777; font-size: 14px; padding: 20px; border-top: 1px solid #e0e0e0; background-color: #f9f9f9; }
...
<body>
  <div class="email-container">
    <div class="header">
      <img src="${process.env.LOGO}" alt="TransBook" width="100" height="100">
      <h2>TOLI-TOLI Admin Invitation</h2>
    </div>
    <div class="body">
      <h1>You're Invited!</h1>
      <p>Hi${name ? ` ${name}` : ''},</p>
      <p>
        You've been invited to join TOLI-TOLI as an admin.<br>
        Please click the button below to set up your account and secure it with 2FA.
      </p>
      <a href="${inviteLink}" class="button">Accept Invitation</a>
      <p style="margin-top:18px; font-size:15px;">
        Or copy and paste this link into your browser:<br>
        <span style="color:#1e88e5">${inviteLink}</span>
      </p>
      <p><strong>Note:</strong> This invitation expires on <b>${expiresAt}</b>.</p>
    </div>
    <div class="footer">
      <p>If you did not expect this email, simply ignore it.</p>
      <p>The TOLI-TOLI Team</p>
    </div>
  </div>
</body>
</html>
...
export { EmailOTP, EmailWelcome, ResendEmail, VerifiedEmail, AdminInviteEmail };